...
import type { GetVideoResponse, VideoRendition, VideoSummary } from "@stream-ops/types";
...
function VideoMetaStrip({ video, renditions }: { video: VideoSummary; renditions: VideoRendition[] }) {
  const maxQuality = renditions.length > 0 ? renditions[0]!.name : null;
  ...
  return (
    <dl className="grid grid-cols-1 gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <div className="flex flex-col gap-2">
        <dt className="text-[11px] font-normal uppercase tracking-[0.22em] text-zinc-500">Max Quality</dt>
        <dd className="font-mono text-sm font-light text-zinc-300">
          {maxQuality ?? (video.status === "ready" ? "—" : "Pending")}
        </dd>
      </div>
...
export function VideoPageClient({
  ...
  const vttUrl = assetBase ? `${assetBase}/videos/${videoId}/thumbnails/thumbnails_lq.vtt` : null;
  const hqVttUrl = assetBase ? `${assetBase}/videos/${videoId}/thumbnails/thumbnails_hq.vtt` : null;
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      <div className="mt-6 border-t border-zinc-800/70 pt-6">
        <VideoMetaStrip video={video} renditions={renditions} />
      </div>
    </div>
  );
}
